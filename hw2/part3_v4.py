...
class HabitatEnvWrapper:
    ...
    def reset_to(self, position, yaw_deg=0.0):
        yaw = math.radians(yaw_deg)
        quat = np.array([0.0, math.sin(yaw / 2.0), 0.0, math.cos(yaw / 2.0)], dtype=np.float32)
        state = habitat_sim.AgentState()
        guess = np.array(position, dtype=np.float32)
        snapped = self.sim.pathfinder.snap_point(guess)
        state.position = snapped  # 確保在 navmesh 上
        state.rotation = quat
        self.agent.set_state(state)

        print(f"[RESET] Agent snapped to navmesh: {snapped}, yaw={yaw_deg}°")
...
def run_navigation(env,
    ...
    ESCAPE_TURN_BATCH = 5  # 連續轉幾步再刷新一次畫面
...
if __name__ == "__main__":
    ...
    # 5) 用「膨脹後」地圖跑 RRT*
    path, nodes = rrt_star_planning(inflated_free, safe_start, safe_goal, SAFE_WEIGHT=500000)
    # 顯示結果
    if path:
