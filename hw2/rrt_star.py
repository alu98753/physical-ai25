...
# RRT* 參數（像素座標系）
# ==========================================================
STEP_SIZE = 10 * DPI//100     # 每次延伸步長（px）
MAX_ITER = 5000     # 迭代上限 TODO ：可以根據距離增加
...
GOAL_REACH_THRESH = 30*STEP_SIZE  # 新節點到目標多少距離內視為可接通
...
# ==========================================================
def smooth_path_chaikin(map_img,path, iterations=2):
    ...
    for _ in range(iterations):
        ...
        for i in range(len(pts) - 1):
            ...
            Q2 = 0.25*P + 0.75*Q
            new_pts += [Q1, Q2]
        if line_collision_free(map_img, P, Q):
...
def smooth_path(map_img, path, iterations=SMOOTH_ITER):
    ...
    while i < n - 1:
        # 嘗試找出從 P[i] 開始，能直接連到最遠的 j
        j = n - 1
        while j > i + 1:
            if line_collision_free(map_img, path[i], path[j]):
                # 成功找到最遠可行段
...
def visualize_multiple_goals(map_path, goals_list, target_class):
    ...
    for i, (x, y) in enumerate(goals_list):
        ax.scatter(x, y, c='red', s=100, zorder=5, marker='X',
                   label="Goal" if i == 0 else None)
        ax.text(x + 5, y + 5, f"{target_class} #{i+1}",
                color='white', fontsize=10,
                bbox=dict(facecolor='red', alpha=0.7, pad=0.1))

    ax.set_title(f"系統找到 {len(goals_list)} 個 '{target_class}' 區域\n請點擊要前往的目標")
    ax.legend()
    ax.axis("equal")
    ...
    def on_click(event):
        ...
        print(f"[CLICK] 你點了 ({click_x:.1f}, {click_y:.1f})")

        # 計算與所有 goal 的距離
        distances = [np.hypot(gx - click_x, gy - click_y) for gx, gy in goals_list]
        nearest_idx = int(np.argmin(distances))
        nearest_goal = goals_list[nearest_idx]
        dist = distances[nearest_idx]
        print(f"[INFO] 離點擊最近的目標: #{nearest_idx+1} {nearest_goal} (距離 {dist:.2f})")
        ...
        selected_goal[0] = nearest_goal
        plt.close(fig)  # 關閉圖窗，結束互動
...
if __name__ == "__main__":
    ...
    MIN_SAFE_DIST = 40.0 * DPI//150  # 絕對最小安全距離（像素）
    ...
    # 執行 RRT* (保持不變)
    path, nodes = rrt_star_planning(binary_map_with_goal, start, goal,MIN_SAFE_DIST, SAFE_WEIGHT=500000)
